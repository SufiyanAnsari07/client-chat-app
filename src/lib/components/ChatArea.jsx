...
const ChatArea = ({user1, user2}) => {
  ...
  useEffect(()=>{
    ...
    if(user2){
      ...
      axios.get(`${API_URL}/api/v1/messages/messages/${user2?._id}`, {
        headers:{
          token
        }
      }).then((res)=>{
        ...
        const allMessages = mergedMessages.sort(
  ...
  if(!user2 || !user1){
    return(
      <div className='w-full h-full flex justify-center items-center'>
        <p className='font-bold text-xl rounded-full p-10 xl:text-5xl text-slate-600 bg-slate-800'>
          No user selected
        </p>
      </div>
    )
  ...
  return(
    <div className='flex flex-col h-full'>
      <div className='flex items-center h-[8.3%] px-2 py-3 border-b-slate-800 border-b-2 justify-between'>
        <div className='w-full flex gap-3 items-center'>
          {user2?.image ? (
            ...
            <FaUserSecret size={40} className='rounded-full cursor-pointer'/>
          )}
            ...
            return(
              <div key={messageData?._id} className='relative w-full'>
                <div className='bg-red-500 absolute px-8 py-3 left-3 rounded-full w-fit rounded-bl-none'>
                  <p className='text-white font-light text-lg max-w-64'>
                    {messageData?.message}
                  </p>
                </div>
              </div>
      ...
      <form
        ...
        className='w-full h-[11%] p-4 bg-slate-800 flex gap-8 items-center'
      >
        <div className='flex flex-col justify-center gap-3 w-[90%] h-[90%]'>
          <input
            ...
            type="text"
            autoComplete='off'
            className='text-4xl bg-transparent outline-none p-8 focus:border h-[90%] focus:border-slate-600 rounded-full'
            placeholder='Message...'
          />
        </div>
